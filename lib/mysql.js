...
export async function todo() {
  const db = GetDatabase();
  const [rows] = await db.query('SELECT * FROM ra_tasks ORDER BY id DESC');
  return rows;
}

/**
 * Ajoute une tâche avec un nouveau todo_id.
 * @param {string} pageName - Le nom de la page.
 * @param {string} label - Le label de la tâche.
...
export async function addTodoTask(pageName, label) {
  ...
  const [maxIdResult] = await db.query(
    'SELECT MAX(todo_id) as maxTodoId FROM ra_tasks'
  );
  const nextTodoId = (maxIdResult[0].maxTodoId ?? 0) + 1;

  const [maxPosResult] = await db.query(
    'SELECT MAX(position) as maxPosition FROM ra_tasks WHERE page_name = ?',
    [pageName]
  );
  const nextPosition = (maxPosResult[0].maxPosition ?? 0) + 1;

  const [result] = await db.query(
    `INSERT INTO ra_tasks (todo_id, label, finished, position, page_name)
     VALUES (?, ?, ?, ?, ?)`,
    [nextTodoId, label, false, nextPosition, pageName]
  );
...
export async function updateTodoTask(todoName, taskId, finished) {
  console.log('updateTodoTask args:', { todoName, taskId, finished });

  const db = GetDatabase();

  const [todos] = await db.query('SELECT id FROM ra_tasks WHERE label = ?', [todoName]);
  if (todos.length === 0) throw new Error('Todo not found');
  const todoId = todos[0].id;

  console.log('finished to write:', finished ? 1 : 0, 'taskId:', taskId, 'todoId:', todoId);

  await db.query(
    'UPDATE ra_tasks SET finished = ? WHERE todo_id = ?',
    [finished ? 1 : 0, taskId]
  );
}

export async function updateTaskPositionsBulk(todoName, positions) {
  const db = GetDatabase(); // ici c’est un pool mysql2/promise

  // Récupérer une connexion dédiée
  const connection = await db.getConnection();

  await connection.beginTransaction();
  try {
    // Récupérer todo_id
    const [todos] = await connection.query('SELECT id FROM ra_tasks WHERE label = ?', [todoName]);
    if (todos.length === 0) throw new Error('Todo not found');
    const todoId = todos[0].id;

    for (const { taskId, position } of positions) {
      await connection.query(
        'UPDATE ra_tasks SET position = ? WHERE id = ? AND todo_id = ?',
        [position, taskId, todoId]
      );
    }
